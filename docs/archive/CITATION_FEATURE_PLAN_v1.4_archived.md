...
| Phase | 標題 | 狀態 | Commit |
|-------|------|------|--------|
| 0-A | documents 擴欄 | ✅ 完成 | `ensureCouncilSchema()` 已含全部 ALTER |
| 0-B | sourceRefs 擴欄 + RAG source_meta | ✅ 完成 | feat(citation): Phase 0 + 1 data foundation |
...
### Phase 0 — 資料地基（Backend Only）✅ 部分完成
...
#### 0-D：Paper Ingest 補充 Section Heading 萃取（已由 0-E 取代）
...
#### 0-E：marker PDF-to-Markdown 微服務（新增）⏳ 待做
...
### Phase 1 — Citation 視覺類型系統（Frontend Only）✅ 完成

**目標：** 三種來源類型用不同顏色底線 + icon 呈現，實線 vs 虛線區分精準度。  
**依賴：** Phase 0-B（需要 `source_type` 和 `is_heuristic` 欄位）  
**改動範圍：** `evidence-annotations.ts` + `evidence-annotated-markdown.tsx`  
**實作摘要：**
- `EvidenceAnnotation` 新增 `sourceType` + `isHeuristic` 欄位
- `SOURCE_TYPE_CONFIG` 定義三種來源的 icon（Lucide）、confirmedColor、heuristicColor、hoverBg
- Hover Tooltip：作者、年份、snippet quote、similarity score
- `EvidenceHoverSpan`：amber/blue/teal 底線，solid/dashed，inline icon 10px
...
### Phase 2 — Citation Popover（Frontend + Phase 0）✅ 完成

**目標：** 點擊 citation underline，展開包含原文上下文的 Popover。  
**依賴：** Phase 0-B（chunk_index）、Phase 0-C（context API）、Phase 1（視覺系統）  
**實作摘要：**
- `citation-popover.tsx`：ReactDOM.createPortal → document.body；位置計算（下方空間不足時自動往上）
- 點擊 citation span → `onCitationClick(annotation, DOMRect)` → `activePopover` state
- Popover 內自動 fetch `/api/documents/[id]/chunks/[n]/context`，顯示 ±2 chunks + 高亮目標段落
- 降級：無 chunk_index 時顯示 snippet quote
- 外部點擊 / Escape 關閉
- `onLocateInDocument` 從 `EvidenceAnnotatedMarkdown` → `AnnotatedInlineText` → `EvidenceHoverSpan` → `CitationPopover` footer 按鈕全鏈路接線
- 同樣 prop 往上穿透：`CollapsibleText` → `AgentMessage` → `DiscussionTimeline`
...
### Phase 3 — 右側面板三 Tab 重構（Frontend）⏳ 待做
...
### Phase 4 — 左側面板 Toggle（Frontend）⏳ 待做
...
### Phase 5 — Citation 智慧層（Backend + Frontend）⏳ 待做
...
```
Phase 0-A  documents 擴欄（authors, year, venue, source_type, markdown_content, marker_processed）
Phase 0-B  CouncilEvidenceSource 擴欄 + RAG source_meta JSON 注入 + extractEvidenceSources 解析
Phase 0-C  GET /api/documents/[id]/chunks/[index]/context → ±2 chunks + sectionHeading
Phase 1    Citation 視覺類型系統（三色底線、solid/dashed、inline icon、hover tooltip）
Phase 2    CitationPopover 元件 + 完整接線（click → popover → fetch context → locate）
```

### ⏳ 下一步建議

```
下一個：Phase 3（3-B + 3-D 先做，3-C 等 0-E）
  3-B：Citations Tab — 升級現有 SourcePanel
        - 作者 + 年份 + source_type icon
        - is_heuristic badge
        - 為 Phase 5 衝突警示預留 UI 位置
  3-D：辯論 Flow Tab（靜態時序連線，現有資料即可）
        - Round 1 → Round 2 時序卡片連線圖
        - 點擊連線高亮左側訊息

之後：Phase 0-E（marker PDF→Markdown）→ 最複雜，需獨立時間
        子任務：
          1. paper-ingest.ts 整合 marker cloud API（POST + poll）
          2. chunk anchor 注入邏輯（Node.js 後處理）
          3. section_heading + char_offset 回填 document_chunks
          4. 新文件測試（arXiv PDF）

完成 0-E 後：Phase 3-C → 原文閱讀 Tab（react-markdown + anchor scroll）

之後：Phase 4 → 左側面板 Toggle（Agent 比較 + 缺口地圖）
之後：Phase 5 → Citation 智慧層（選做，依需求決定）
```

**並行可能性：** Phase 0-E（後端 marker 服務）可與 Phase 3-B、3-D（純前端）並行開發，互不干擾。

---

## 2026-05-02 Supplement: Implementation Reality and Priority Order

### 1. Current Code Reality

This plan describes the target citation system well, but the codebase is not starting from zero. Some core runtime and UI groundwork already exists, so execution should follow actual repo state rather than assume every phase is still blank.

#### 1.0 Reality audit status by phase

| Phase | Audited status | Notes |
|------|----------------|-------|
| 0-A | done | `documents` schema already includes `authors`, `year`, `venue`, `source_type`, `markdown_content`, `marker_processed` via `ensureCouncilSchema()` |
...
| 5-B | not started | no uncited-claim highlighting layer exists |
| 5-C | not started | no `responds_to_turn_id` data model or precise debate-graph backend support exists |

#### 1.0.1 Immediate conclusion from the audit
...
#### 1.1 Already present or partially present

- Debate runtime already extracts `sourceRefs` from tool results and streams them to the UI.
- Tool evidence is already persisted in `council_evidence`.
- Inline citation/evidence rendering already exists in:
  - `src/lib/evidence-annotations.ts`
  - `src/components/council/evidence-annotated-markdown.tsx`
- Source panel rendering already exists in:
  - `src/components/council/source-panel.tsx`
- Current citation UX already supports:
  - marker-based mapping when `[n]` markers exist
  - heuristic sentence-to-source matching when explicit markers do not exist
  - hover tooltip / click-through source inspection
- Chunk-based RAG retrieval already exists, so citation does not need a brand-new evidence foundation.

#### 1.2 Still incomplete or high risk

- The final metadata contract still needs audit confirmation across runtime, DB, hydration, and UI:
  - `source_type`
  - `is_heuristic`
  - `similarity_score`
  - `chunk_index`
  - `document_id`
- `documents` and `document_chunks` may not yet fully match the fields assumed by this plan.
- Marker PDF-to-Markdown ingestion remains the highest-risk area:
  - external API dependency
  - ingestion retry / backfill
  - schema migration
  - chunk-to-markdown anchor alignment
- Source Reader depends on marker markdown, so it should not block lower-risk citation UI work.
- Conflict analysis / uncited claim / coverage analysis should be postponed until the metadata contract is stable.

#### 1.3 Progress update after implementation

- `3-B` first-pass integration is now complete:
  - right sidebar tabs are `Citations / Flow / Chat`
  - source interactions route back to the `Citations` tab
  - `SourcePanel` now renders `Document / Academic / Web` and heuristic status
- `3-D` first-pass integration is now complete:
  - `Flow` tab mounts `debate-map.tsx`
  - `Flow` is disabled when Round 2 data is absent
- `0-B / 0-C` contract closure is now complete:
  - live SSE path preserves citation metadata in `src/lib/council-event-reducer.ts`
  - persisted reload path preserves citation metadata in `src/lib/services/council-session-hydrator.ts`
  - targeted tests cover both paths:
    - `src/__tests__/council-event-reducer.test.ts`
    - `src/__tests__/council-session-hydrator.test.ts`
- `0-E` foundation is now partially implemented:
  - `ingestPaper()` accepts `sourceType` and optional `pdfBuffer`
...
- Deliver low-risk, user-visible workflow improvements first.
- Isolate the marker pipeline as its own milestone.
- Only build analysis features after the evidence contract is stable.

#### Priority 1: Audit the current implementation and lock the data contract

Goal:

- Mark each planned phase as `done`, `partial`, or `not started` against the real repo state.
- Lock the final citation/evidence contract before expanding more UI surfaces.

Audit targets:

- `CouncilEvidenceSource` shape in runtime, DB hydration, and UI
- `extractEvidenceSources()` actual output behavior
- `documents` / `document_chunks` schema state
- reducer / hydrator / source panel / inline annotation compatibility

Expected output:

- a plan-vs-code checklist
- a migration/type completion list

#### Priority 2: Deliver the right-panel workflow first

Build first:

- `Phase 3-B` Citations Tab
- `Phase 3-D` Debate Flow Tab

Why:

- These can be built on top of existing `messages`, `sourceRefs`, and round structure.
- They do not require marker markdown to be useful.
- They create immediate UX value while heavier backend work remains isolated.

Implementation note:

- Reuse the existing `SourcePanel` rather than rewriting citations from scratch.
- Flow view should first focus on round grouping, seat sequence, and evidence highlight, not conflict inference.

#### Priority 3: Close the Phase 0 metadata gaps

Focus:

- `0-A` document metadata audit / migration
- `0-B` evidence source metadata completion
- `0-C` context API audit / completion

Why:

- These fields are prerequisites for Phase 1, 2, and 3 quality.
- If the contract is unstable, every panel will need rework.

Minimum fields to stabilize:
...
Preferred if practical in the same pass:

- `authors`
- `year`
- `venue`

#### Priority 4: Treat marker ingestion as an independent milestone

This should not be bundled with general UI work.

Includes:

- marker cloud API integration
- `paper-ingest.ts` extension
- `documents.markdown_content`
- `documents.marker_processed`
- `document_chunks.section_heading`
- `document_chunks.char_offset`
- retry / timeout / backfill / degraded fallback

Why:

- This is the highest-risk item in the entire plan.
- If bundled with lighter UI phases, it will delay the whole citation feature set.

#### Priority 5: Build Source Reader after the marker path is defined
...
- If marker markdown exists, use full markdown reader + anchor scroll.
- If marker markdown is unavailable, degrade to a chunk-context reader.

Why:

- This avoids making `3-C` fully blocked on `0-E`.
- Users can still inspect cited chunk context before the full PDF-reader experience is complete.

#### Priority 6: Build analysis features last

Do last:

- `Phase 4-B` Agent Compare
- `Phase 4-C` Gap Map
- `Phase 5-A` citation conflict
- `Phase 5-B` uncited claims
- `Phase 5-C` responds-to / graph precision

Why:

- These rely on stable metadata, chunk linking, and source reader behavior.
- Building them early increases rework risk substantially.

### 3. Recommended Milestones

#### Milestone A: Reality Audit + Contract Lock

- Audit repo state
- Mark plan items as actual `done / partial / not started`
- Finalize type/schema checklist

#### Milestone B: Citation Workspace v1

- Complete `Citations` tab
- Complete `Debate Flow` tab
- Normalize right-panel state and interactions

#### Milestone C: Evidence Metadata Completion

- Fill `source_type`, `is_heuristic`, `chunk_index`, `similarity_score`
- Verify runtime -> DB -> hydration -> UI consistency

#### Milestone D: Marker Ingestion

- marker cloud integration
- schema updates
- backfill / retry / fallback

#### Milestone E: Source Reader

- markdown reader
- anchor scroll
- chunk-context fallback

#### Milestone F: Analysis Layer
...
### 4. Recommended Next Practical Sequence

1. Audit the repo and annotate this plan with real completion status.
2. Ship `3-B Citations Tab` and `3-D Debate Flow Tab`.
3. Close `0-B` / `0-C` metadata and API gaps.
4. Build `0-E` marker ingestion as a separate track.
5. Build `3-C Source Reader Tab` with graceful fallback.
6. Build `4` / `5` analysis features last.

### 5. What should not be coupled
